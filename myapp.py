...
def update_coin_data():
    ...
    #Dataframe 뿌려주기(초기값)
    coin_df = pd.DataFrame({'Name': coin_array_noKRW, 'Price(￦)': trade_price, 'Trd': up_down, 'Change(%)': signed_change_rate, 'Change(￦)': signed_change_price, 'A/B': sum_ask_bid_rate, 'Ask': sum_ask_size, 'Cmpr': compare, 'Bid': sum_bid_size})
